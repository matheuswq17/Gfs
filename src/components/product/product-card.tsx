...
export function ProductCard({ product }: { product: ProductWithRelations }) {
  ...
  return (
    <article className="motion-card feature-card group flex h-full flex-col rounded-lg border border-[#dbe4f0] bg-white shadow-sm hover:border-[#b9c8db]">
      <Link href={`/produtos/${product.slug}`} className="block overflow-hidden rounded-t-lg bg-[#f3f7fc]">
        <Image
          src={image}
          alt={product.images[0]?.alt || product.name}
          width={720}
          height={560}
          className="aspect-[1.2] w-full object-cover transition duration-300 group-hover:scale-[1.03]"
        />
      </Link>
